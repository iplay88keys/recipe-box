...
func (r *RecipeRepository) List() ([]*Recipe, error) {
	rows, err := r.db.Query(listRecipesQuery)
	if err != nil {
		fmt.Printf("Failed to fetch recipes: %s", err.Error())
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.Name, &r.Description); err != nil {
			fmt.Printf("Failed to scan recipes: %s", rows.Err())
			return nil, errors.New("failed to scan recipes")
		}
		recipes = append(recipes, r)
	}
	if rows.Err() != nil {
		fmt.Printf("Failed to loop through recipes: %s", rows.Err())
...
func (r *RecipeRepository) Get(id int) (*Recipe, error) {
	...
	if err := row.Scan(&recipe.ID,
		...
		if err == sql.ErrNoRows {
			fmt.Printf("Could not find recipe '%d': %s", id, err.Error())
			return nil, errors.New("could not find recipe")
		}

		fmt.Printf("Failed to scan recipe '%d': %s", id, err.Error())
...
func (r *RecipeRepository) Insert(recipe *Recipe, userID int64) (int64, error) {
	...
	if err != nil {
		fmt.Printf("Recipe could not be saved: %s", err.Error())
		return 0, errors.New("recipe could not be saved")
	}

	id, err := res.LastInsertId()
	if err != nil {
		fmt.Printf("Recipe was not saved correctly: %s", err.Error())
